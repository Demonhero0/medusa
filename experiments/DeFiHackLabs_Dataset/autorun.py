...
def run_smartfitness(config_type, t, i, total, name, block, addresses):
    print(f'Running {config_type} {t} {i+1}/{total}: {name}')
    ...
    if os.path.exists(directory):
        print(f'{config_type} {t} {i+1}/{total}: {name} already exists, skipping')
        return
    block = int(block)
    ...
    if os.path.exists(abi_path):
        subprocess.run(['ln', '-s', abi_path, f'{directory}/abis'])
...
with open('./dapps.csv', 'r', encoding='utf-8') as file:
    ...
    #                 executor.submit(run_smartfitness, config_type, t, i, len(rows), *row)
