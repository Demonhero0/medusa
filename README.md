...
```
./smartseed fuzz --config example/config.json --compilation-target example.sol --target-contracts Example
```
...
```
./smartseed fuzz --config example/config_onchain.json
```
...
`smartseed` is licensed and distributed under the [AGPLv3](./LICENSE).